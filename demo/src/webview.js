...
module.exports = function (context) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("demo.openWebview", function (uri) {
      // 创建webview
      ...
      const result = vscode.workspace
        .getConfiguration()
        .get("Mino插件.yourName");
      panel.webview.html = `<html><body>你好，我是${result}</body></html>`;
    })
  );
  // /_\: 将package.json的activationEvents设为*，可以开启vs时立即执行该插件
  const key = "Mino插件.showTip";
  // 如果设置里面开启了欢迎页显示，启动欢迎页
  if (vscode.workspace.getConfiguration().get(key)) {
    vscode.commands.executeCommand("demo.openWebview");
  }
  // context.subscriptions.push(
  //   vscode.commands.registerCommand("demo.openWebview", function (uri) {
